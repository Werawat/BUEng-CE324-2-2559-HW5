...
function fillScene() {
	...
	var robotNewLeftMaterial = new THREE.MeshPhongMaterial( { color: 0x099909, specular: 0x555533, shininess: 100 } );
	...
	toparm = new THREE.Object3D();
	var handLength = 80;
	createRectangle( toparm, handLength, robotNewLeftMaterial );
	// Move the hand part to the end of the forearm.
	toparm.position.y = faLength;
	forearm.add( toparm );
...
function createRectangle( part, length, material )
{
	var box = new THREE.Mesh(
		new THREE.CubeGeometry( 10, length, 10 ), material );
	box.position.y = length/2;
	part.add( box );


	var sphere = new THREE.Mesh(
		new THREE.SphereGeometry( 30, 32, 16 ), material );
...
function init() {
	...
	camera.position.set( -510, 350, 100 );
	// CONTROLS
...
function render() {
	...
	toparm.rotation.y = effectController.ny * Math.PI/180;	// yaw
	toparm.rotation.z = effectController.nz* Math.PI/180;;	// translate
...
function setupGui() {
	...
	h.add(effectController, "ny", -180.0, 180.0, 0.025).name("New Hand y");
	h.add(effectController, "nz", -45.0, 45.0, 0.025).name("New Hand z");
}
